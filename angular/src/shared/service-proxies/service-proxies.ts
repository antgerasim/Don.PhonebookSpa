        ...
        return this.http.request(url_, options_).flatMap((response_) => {
            ...
            if (response_ instanceof Response) {
                try {
                    ...
                    return <Observable<ListResultDtoOfPersonListDto>><any>Observable.throw(e);
                }
            } else
                return <Observable<ListResultDtoOfPersonListDto>><any>Observable.throw(response_);
        });
    }

    protected processGetPeople(response: Response): Observable<ListResultDtoOfPersonListDto> {
        const status = response.status; 
        ...
        if (status === 200) {
            ...
            let result200: any = null;
            let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
            result200 = resultData200 ? ListResultDtoOfPersonListDto.fromJS(resultData200) : new ListResultDtoOfPersonListDto();
            return Observable.of(result200);
        } else if (status !== 200 && status !== 204) {
            const _responseText = response.text();
            return throwException("An unexpected server error occurred.", status, _responseText, _headers);
